	...
	$('#grid').datagrid({
		...
		columns:[[
					...
					{field:'-',title:'操作',width:200,formatter: function(value,row,index){
						var oper = "<a href=\"javascript:void(0)\" onclick=\"updatePwd_reset(\'" + row.username + '\')">重置密码</a>';
						return oper;
					}}
					]],
		singleSelect: true,
...
//打开重置密码窗口
function updatePwd_reset(username){
	console.log("updatePwd_reset : "+username)
	$('#editDlg').dialog('open');
	//清空表单
	 $('#editForm').form('clear');
	 //加载数据
	 $('#editForm').form('load',{username: username, newPwd:""});
}